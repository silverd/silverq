...
class Com_Queue_Adapter_MemcacheQ implements Com_Queue_Adapter_Interface
{
    /**
     * 连接实例
     *
    ...
    public function __construct($queueName, array $config)
    {
        ...
        // 连接实例
        $this->_memcache = new Com_Cache_Memcache($configGroup);

        // 初始化键名
        $this->_pushedCountKey = 'QUEUE:' . $queueName . ':PushedCount'; // 已压进元素数
        $this->_popedCountKey  = 'QUEUE:' . $queueName . ':PopedCount';  // 已弹出元素数
        $this->_queueDataKey   = 'QUEUE:' . $queueName . ':Data';        // 队列数据前缀
    }
